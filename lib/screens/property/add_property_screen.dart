...
class _AddPropertyScreenState extends State<AddPropertyScreen> {
  ...
  // Add these methods to _AddPropertyScreenState:
  ...
  Future<void> _searchPlaces(String query) async {
    ...
    try {
      final suggestions = await LocationService().searchPlaces(query);
      setState(() {
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Expanded(child: _buildPriceTypeDropdown()),
            ],
          ),
        ],
      ),
  ...
  Future<void> _getCurrentLocationForAddress() async {
    ...
    try {
      ...
      if (locationProvider.currentLocation != null) {
        ...
        // Get address from coordinates
        final address = await locationProvider.getFullAddress(
          _latitude,
          _longitude,
        );
        if (address != null) {
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            runSpacing: 12,
            children:
                FurnishingType.values.map((type) {
  ...
  Future<void> _pickVirtualTourImages() async {
    try {
      ...
      if (images.isNotEmpty) {
        setState(() {
          _virtualTourImages.addAll(
            images.map((image) => File(image.path)).toList(),
          );
        });
        print(
          'Added ${images.length} virtual tour images. Total: ${_virtualTourImages.length}',
        );
      }
    } catch (e) {
  ...
  Future<void> _pickImages() async {
    try {
      ...
      if (images.isNotEmpty) {
        ...
        print(
          'Added ${images.length} regular images. Total: ${_selectedImages.length}',
        );
      }
    } catch (e) {
  ...
  Future<void> _addProperty(
    ...
    try {
      ...
      final success = await propertyProvider.addProperty(
        ...
        virtualTourImages:
            _virtualTourImages.isNotEmpty ? _virtualTourImages : null,
      );
